...
def init_particles_freespace(num_particles, occupancy_map):
    ...
    for i in range(num_particles): 
        ...
        # generate a random number between 0 and len(x_empty)

        random_idx = np.random.choice(len(x_empty))

        # use that random number as index 

        x0_vals[i] = x_empty[random_idx]*10

        y0_vals[i] = y_empty[random_idx]*10
